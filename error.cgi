...
if File.exists?($conf.errorlog)
	...
	subject = _("Bug in MezBOX-Umfrage")

	$d << _("An error occurred while executing MezBOX-Umfrage.<br/>Please send an error report, including your browser, operating system, and what you did to %{admin}.") % {:admin => "<a href='mailto:#{$conf.bugreportmail}?subject=#{CGI.escape(subject)}&amp;body=#{CGI.escape(errormessagebody)}'>#{$conf.bugreportmail}</a>"}
...
if $conf.auto_send_report && !known
	...
	%x{mail -s "Bug in MezBOX-Umfrage" #{$conf.bugreportmail} < #{tmpfile}}
